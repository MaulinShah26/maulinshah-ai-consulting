const CATS = [
  { y: 70, c: "#5B3FBE", cat: "Puppy starter", items: ["Puppy food", "Training treats", "Chew toys"] },
  { y: 150, c: "#8B73D8", cat: "Senior care", items: ["Joint supplements", "Soft food", "Dental chews"] },
  { y: 230, c: "#4F635D", cat: "Grooming", items: ["Shampoo", "Brush", "Nail clippers"] },
];
...
export function AffinityMap() {
  return (
    <svg viewBox="0 0 720 300" className="block w-full h-auto" aria-hidden>
      ...
      <text x="88" y="150" textAnchor="middle" className="font-serif" fontSize="13" fontWeight="600" fill="#14201D">Bought together</text>
      {CATS.map((c, i) => (
        <g key={i}>
