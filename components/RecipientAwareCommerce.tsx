...
const profiles: Record<RecipientId, Profile> = {
  ...
  dad: {
    ...
    color: "bg-emerald-100 text-[#31410D]"
  },
  mom: {
    id: "mom",
    label: "Mom",
...
export default function RecipientAwareCommerce() {
  ...
  return (
    <main className="min-h-screen bg-[#F5F5F1] text-[#20211F]">
      <CommerceHeader
        query={query}
        setQuery={setQuery}
        onSearch={search}
...
function CommerceHeader({
  ...
  return (
    <>
      <header className="border-b border-black/[0.06] bg-[#FCFCF9]">
        <div className="mx-auto flex max-w-[1500px] items-center gap-4 px-4 py-4 sm:px-6">
          <button
            onClick={() => onNavigate("home")}
            className="flex shrink-0 items-center gap-2.5"
          >
            <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#20211F] text-xs font-black text-[#DFF168]">
              V
            </span>
            <span className="hidden sm:block">
              <span className="block text-sm font-black tracking-[0.08em]">VELA</span>
              <span className="mt-0.5 block text-[9px] font-medium text-black/35">contextual shopping</span>
            </span>
          </button>

          <form
            onSubmit={(event) => onSearch(event)}
            className="mx-auto flex min-w-0 max-w-3xl flex-1 items-center gap-2 rounded-2xl border border-black/[0.07] bg-[#F3F3EF] px-3"
          >
            <Search className="h-4 w-4 shrink-0 text-black/35" />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              className="min-w-0 flex-1 bg-transparent py-3 text-sm text-[#20211F] outline-none"
              placeholder="Search products"
            />
            <button
              type="submit"
              className="rounded-xl bg-[#20211F] px-3 py-2 text-[10px] font-bold text-white"
            >
              Search
            </button>
          </form>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 rounded-2xl border border-black/[0.07] bg-white px-2.5 py-2"
            >
              <span className={"grid h-8 w-8 place-items-center rounded-full text-[10px] font-bold " + profile.color}>
                {profile.initials}
              </span>
              <span className="hidden text-left md:block">
                <span className="block text-[9px] font-medium text-black/35">Shopping for</span>
                <span className="block text-xs font-bold">{profile.label}</span>
              </span>
              <ChevronDown className="hidden h-3.5 w-3.5 text-black/35 md:block" />
            </button>
          ...
          <button
            ...
            className="hidden rounded-xl px-2.5 py-2 text-left text-xs font-bold text-black/55 hover:bg-black/[0.03] lg:block"
          >
            <span className="block text-[9px] font-medium text-black/30">Profiles &</span>
            memory
          </button>

          <button
            onClick={onCart}
            className="relative grid h-11 w-11 place-items-center rounded-2xl bg-[#20211F] text-white"
            aria-label="Open shopping bag"
          >
            <ShoppingCart className="h-5 w-5" />
            {cartCount ? (
              <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-[#DFF168] px-1 text-[9px] font-black text-[#20211F]">
                {cartCount}
              </span>
            ) : null}
          </button>
      ...
      <nav className="border-b border-black/[0.06] bg-white">
        <div className="mx-auto flex max-w-[1500px] items-center gap-1 overflow-x-auto px-4 py-2.5 sm:px-6">
          {["Mobiles", "Electronics", "Fashion", "Home", "Beauty", "Appliances", "Grocery", "Pet care", "Deals"].map(
            (category) => (
              <button
                key={category}
                onClick={() => category === "Mobiles" && onNavigate("results")}
                className="whitespace-nowrap rounded-full px-3 py-1.5 text-[11px] font-semibold text-black/55 hover:bg-[#F3F3EF] hover:text-black"
              >
                {category}
              </button>
            )
      ...
      <div className="border-b border-black/[0.06] bg-[#F9F9F6]">
        <div className="mx-auto flex max-w-[1500px] items-center gap-2 overflow-x-auto px-4 py-2.5 sm:px-6">
          <span className="shrink-0 text-[9px] font-bold uppercase tracking-[0.12em] text-black/30">
            Current context
          </span>
          <button
            onClick={() => setRecipientMenuOpen(!recipientMenuOpen)}
            className="flex shrink-0 items-center gap-2 rounded-full bg-[#20211F] px-2.5 py-1.5 text-[10px] font-semibold text-white"
          >
          ...
          <span className="shrink-0 text-[10px] text-black/25">·</span>
...
function RecipientMenu({
  ...
  return (
    <div className="absolute right-0 top-[56px] z-50 w-[320px] overflow-hidden rounded-3xl border border-black/[0.08] bg-[#FCFCF9] text-[#20211F] shadow-2xl">
        ...
        {(Object.keys(profiles) as RecipientId[]).map((id) => {
          ...
          return (
            <button
              ...
              className={
                "flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left hover:bg-black/[0.03] " +
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-[10px] text-black/30">
                  {item.temporary ? "Temporary recipient" : item.saved.join(" · ")}
                </span>
              </span>
              {id === recipientId ? <Check className="h-4 w-4 text-[#526C1D]" /> : null}
            </button>
          );
        })}
      </div>
      <button className="flex w-full items-center gap-2 border-t border-black/[0.05] px-4 py-3 text-xs font-semibold text-[#405418]">
...
function HomeView({
  ...
  return (
    <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
      <section className="grid overflow-hidden rounded-[32px] border border-black/[0.06] bg-[#ECEFE8] lg:grid-cols-[1.1fr_0.9fr]">
        <div className="p-6 sm:p-8 lg:p-10">
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.14em] text-[#526C1D]">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {profile.saved.slice(0, 4).map((item) => (
              <span
                key={item}
                className="rounded-full border border-emerald-900/10 bg-white/70 px-3 py-1.5 text-[10px] font-semibold text-black/65"
              >
                {item}
              </span>
            ))}
        ...
        <div className="relative min-h-[340px] bg-[#E7F5B8]">
          {top ? (
            <img
              src={top.image}
      ...
      <section className="mt-5 rounded-2xl border border-black/[0.07] bg-white p-4 sm:p-5">
              ...
              return (
                <button
                  ...
                  className={
                    ...
                    (id === recipientId
                      ? "border-[#526C1D] bg-[#F1F6DF] text-[#31410D]"
                      : "border-black/[0.07] bg-white text-black/55")
                  }
                >
      ...
      <section className="mt-7">
        <div className="flex items-end justify-between gap-3">
          ...
          <button onClick={onSearch} className="text-xs font-semibold text-[#405418]">
  ...
  return (
    <div className="mx-auto max-w-[1500px] px-4 py-4 sm:px-6">
      ...
      <div className="mt-3 grid gap-5 xl:grid-cols-[250px_minmax(0,1fr)]">
        <aside className="hidden self-start overflow-hidden rounded-3xl border border-black/[0.07] bg-white xl:block">
        ...
        <section className="min-w-0">
          <div className="border border-black/[0.07] bg-white">
            <div className="border-b border-black/[0.05] p-4 sm:p-5">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                ...
                <select className="rounded-2xl border border-black/[0.07] bg-white px-3 py-2 text-xs font-semibold">
            ...
            <button
              ...
              className="flex w-full items-center justify-between gap-4 border-b border-black/[0.07] bg-[#f8fbf9] p-4 text-left"
            >
              <span>
                <span className="block text-[10px] font-bold uppercase tracking-[0.14em] text-[#405418]">
              ...
              <ChevronDown className={"h-4 w-4 shrink-0 text-black/30 transition " + (missionOpen ? "rotate-180" : "")} />
            </button>
            ...
            {missionOpen ? (
              <div className="border-b border-black/[0.07] p-4 sm:p-5">
                <div className="grid gap-4 lg:grid-cols-[1fr_330px]">
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-[0.12em] text-black/30">
                    ...
                    <textarea
                      ...
                      className="mt-2 w-full resize-none rounded-2xl border border-black/[0.07] p-3 text-sm leading-6 outline-none focus:border-[#90A94E]"
                    />
                    <button
                      onClick={onApplyMission}
                      className="mt-2 rounded-2xl bg-[#20211F] px-4 py-2.5 text-xs font-semibold text-white"
                    >
                  ...
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-black/30">
                      Interpreted context
                    </div>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {(Object.entries(weights) as Array<[PriorityKey, number]>)
                        ...
                        .map(([key, weight]) => (
                          <span
                            key={key}
                            className="rounded-full bg-[#F1F6DF] px-2.5 py-1.5 text-[10px] font-semibold text-[#405418]"
                          >
                            {priorityLabels[key]} · {weight === 5 ? "high" : "medium"}
                          </span>
                        ))}
                      <span className="rounded-full bg-[#ECECE7] px-2.5 py-1.5 text-[10px] font-semibold text-black/55">
                        Budget {money(budget)}
                      </span>
                    </div>
                    <p className="mt-3 text-[10px] leading-4 text-black/30">
          ...
          <div className="mt-3 border border-black/[0.07] bg-white p-4">
          ...
          <div className="mt-4 rounded-3xl border border-black/[0.07] bg-white p-3">
            <div className="flex flex-col gap-2 border-b border-black/[0.05] p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
              ...
              {top ? (
                <div className="rounded-full bg-[#F1F6DF] px-3 py-1.5 text-[10px] font-semibold text-[#405418]">
...
function SearchResultCard({
  ...
  return (
    <article className="grid gap-4 rounded-[24px] border border-black/[0.06] bg-[#FCFCF9] p-4 sm:grid-cols-[220px_minmax(0,1fr)] sm:p-5">
      <button onClick={onOpen} className="relative overflow-hidden rounded-2xl bg-[#F3F3EF]">
        <img
          src={product.image}
      ...
      <div className="min-w-0">
        <div className="flex items-start gap-3">
          <button onClick={onOpen} className="min-w-0 flex-1 text-left">
            <div className="text-[10px] uppercase tracking-[0.12em] text-black/30">{product.brand}</div>
            <h2 className="mt-1 text-lg font-semibold leading-6 hover:text-[#405418]">
              {product.name} 5G Smartphone, {product.displayText}, {product.batteryText}
            </h2>
          </button>
          <button onClick={onSave} className="p-1.5 text-black/30" aria-label="Save product">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-black/45">
          <span className="rounded bg-[#526C1D] px-1.5 py-0.5 font-bold text-white">{product.rating} ★</span>
        ...
        <div className="mt-3 flex flex-wrap items-baseline gap-2">
          ...
          <span className="text-xs text-black/30 line-through">{money(product.mrp)}</span>
          <span className="text-xs font-semibold text-[#526C1D]">
            {Math.round((1 - product.price / product.mrp) * 100)}% off
          </span>
        </div>

        <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_320px]">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] text-black/55">
          ...
          <div className="rounded-xl border border-[#DDE8B5] bg-[#F1F6DF] p-3">
            <div className="flex items-center justify-between gap-3">
              <span className="text-xs font-bold text-[#31410D]">
                {rank === 1 ? "Best fit for " + profile.label : "Fit for " + profile.label}
              </span>
              <span className="text-[10px] font-bold text-[#526C1D]">{score}%</span>
            </div>
            <div className="mt-2 space-y-1">
              {fit.matches.slice(0, 3).map(([key]) => (
                <div key={key} className="flex items-center gap-1.5 text-[10px] text-[#405418]">
            ...
            <button
              onClick={onOpen}
              className="mt-2 text-[10px] font-bold text-[#31410D] underline underline-offset-2"
            >
              Why this fits
            </button>
          </div>
...
function StoreProductCard({
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-black/[0.07] bg-white">
      <div className="relative bg-[#F3F3EF]">
        ...
        <button
          ...
          aria-label="Save"
        >
          <Heart className={"h-4 w-4 " + (saved ? "fill-rose-500 text-rose-500" : "text-black/30")} />
        </button>
      </div>
      <div className="p-3.5">
        <div className="text-[9px] font-bold uppercase tracking-[0.12em] text-black/30">{product.brand}</div>
        <button onClick={onOpen} className="mt-1 text-left text-sm font-semibold hover:text-[#405418]">
          {product.name}
        </button>
        ...
        <div className="mt-2">
          ...
          <span className="ml-2 text-[10px] text-black/30 line-through">{money(product.mrp)}</span>
        </div>
        <div className="mt-3 rounded-lg bg-[#F1F6DF] p-2.5">
          <div className="flex items-center justify-between text-[10px] font-bold text-[#31410D]">
...
function ProductDetail({
  ...
  return (
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-white">
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/[0.07] bg-white px-4 py-3 sm:px-6">
        <div className="text-sm font-bold">Product details</div>
        <button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-[#ECECE7]" aria-label="Close">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="mx-auto max-w-[1350px] px-4 py-6 sm:px-6">
        ...
        <div className="mt-4 grid gap-7 lg:grid-cols-[520px_minmax(0,1fr)]">
          <div className="self-start lg:sticky lg:top-20">
            <div className="overflow-hidden rounded-xl border border-black/[0.07] bg-[#F3F3EF]">
              <img src={product.image} alt={product.brand + " " + product.name} className="aspect-square w-full object-contain" />
            </div>
            <div className="mt-3 grid grid-cols-4 gap-2">
              {[0, 1, 2, 3].map((index) => (
                <div key={index} className="overflow-hidden rounded-2xl border border-black/[0.07] bg-white">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-black/30">{product.brand}</div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-black/45">
              <span className="rounded bg-[#526C1D] px-2 py-1 font-bold text-white">{product.rating} ★</span>
              <span>{product.reviews.toLocaleString("en-IN")} ratings</span>
              <span>·</span>
              <span>Free delivery</span>
            </div>

            <div className="mt-4 border-y border-black/[0.07] py-4">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-sm font-semibold text-[#526C1D]">
                  {Math.round((1 - product.price / product.mrp) * 100)}% off
                </span>
              </div>
              <p className="mt-1 text-xs text-black/45">Inclusive of all taxes</p>
            </div>

            <section className="mt-5 rounded-2xl border-2 border-[#D4E5A0] bg-[#F1F6DF] p-4 sm:p-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-[#405418]">
                    <Sparkles className="h-4 w-4" />
                ...
                <span className="rounded-full bg-white px-3 py-1.5 text-[10px] font-semibold text-[#31410D]">
                  Shopping for {profile.label}
                </span>
              </div>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                {fit.matches.map(([key]) => (
                  <div key={key} className="flex items-center gap-2 rounded-lg bg-white p-3 text-xs text-[#31410D]">
              ...
              {product.unknown ? (
                <div className="mt-3 rounded-2xl border border-black/[0.07] bg-white p-3 text-xs text-black/55">
                  <strong className="text-[#20211F]">Unknown:</strong> {product.unknown}. This should remain unknown rather than be inferred as fact.
                </div>
              ) : null}
            </section>
            ...
            <section className="mt-5 border-t border-black/[0.07] pt-5">
              <div className="grid gap-3 sm:grid-cols-[1fr_220px]">
                <div className="rounded-xl bg-[#F3F3EF] p-4">
                  ...
                  <div className="mt-2 inline-flex rounded-full border border-black/[0.07] bg-white px-2.5 py-1 text-[10px] font-semibold text-black/55">
                    {memoryMode === "remember"
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#405418]">
            Profiles & memory
          </div>
        ...
        <button className="rounded-2xl border border-black/[0.1] bg-white px-4 py-2.5 text-xs font-semibold">
          + Add person or pet
        </button>
      </div>
          ...
          return (
            <section
              ...
              className={
                ...
                (id === recipientId ? "border-[#90A94E] ring-2 ring-[#E4ECC8]" : "border-black/[0.07]")
              }
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] text-black/30">{profile.relation}</div>
                </div>
              ...
              {profile.saved.length ? (
                <>
                  <div className="mt-4 text-[10px] font-bold uppercase tracking-[0.12em] text-black/30">
                    Remembered preferences
                  </div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {profile.saved.map((item) => (
                      <span key={item} className="rounded-full bg-[#ECECE7] px-2.5 py-1.5 text-[10px] font-semibold text-black/55">
                        {item}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mt-4 rounded-2xl border border-black/[0.07] p-3">
                <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-black/30">
                  Learning control
                </div>
                <select
              ...
              <div className="mt-4 flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-2xl bg-[#20211F] px-3 py-2.5 text-xs font-semibold text-white"
                >
                  Shop for {profile.label}
                </button>
                <button className="rounded-2xl border border-black/[0.1] px-3 py-2.5 text-xs font-semibold">
                  Edit
                </button>
              </div>
            </section>
      ...
      <div className="mt-6 rounded-2xl border border-black/[0.07] bg-white p-5">
        <div className="flex items-center gap-2 text-sm font-bold">
...
function CartDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[90] flex justify-end bg-black/45" onMouseDown={onClose}>
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/[0.07] bg-white p-4">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#405418]">
              Cart
            </div>
        ...
        {cart.length ? (
          <div className="p-4">
            <div className="space-y-4">
              {cart.map((item, index) => {
                ...
                return (
                  <div key={item.productId + "-" + index} className="rounded-2xl border border-black/[0.07] p-3">
                    <div className="flex gap-3">
                      <img src={product.image} alt="" className="h-24 w-24 rounded-2xl bg-[#F3F3EF] object-contain" />
                    ...
                    <div className="mt-3 grid gap-2 rounded-2xl bg-[#F3F3EF] p-3">
                      <label className="text-[9px] font-bold uppercase tracking-[0.12em] text-black/30">
                        Who is this for?
                      </label>
                      <select
            ...
            <div className="mt-5 border-t border-black/[0.07] pt-4">
              ...
              <button
                onClick={onCheckout}
                className="mt-4 w-full rounded-2xl bg-[#20211F] py-3 text-sm font-bold text-[#20211F]"
              >
                Proceed to simulated checkout
              </button>
              <p className="mt-2 text-center text-[10px] leading-4 text-black/30">
          ...
          <div className="grid min-h-[70vh] place-items-center p-6 text-center">
            <div>
              ...
              <p className="mt-1 text-xs text-black/45">Add a phone to see recipient attribution continue into cart.</p>
            </div>
          </div>
        )}
      </aside>
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-black/50 p-4" onMouseDown={onClose}>
      ...
      >
        ...
        {product ? (
          <div className="mt-4 flex items-center gap-3 rounded-2xl border border-black/[0.07] p-3">
            <img src={product.image} alt="" className="h-20 w-20 rounded-2xl bg-[#F3F3EF] object-contain" />
            <div>
              <div className="text-[9px] font-bold uppercase text-black/30">{product.brand}</div>
              <div className="text-sm font-semibold">{product.name}</div>
              <div className="mt-1 text-[10px] text-black/45">
        ...
        {!done ? (
          <>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button onClick={() => setDone(true)} className="rounded-lg border border-[#C9DD83] bg-[#F1F6DF] py-3 text-xs font-semibold text-[#31410D]">
                Worked well
              </button>
              <button onClick={() => setDone(true)} className="rounded-2xl border border-black/[0.07] py-3 text-xs font-semibold">
                Some issues
              </button>
            </div>
          </>
        ) : (
          <div className="mt-4 rounded-xl bg-[#F1F6DF] p-4">
            <div className="flex items-center gap-2 text-sm font-bold text-[#31410D]">
              <Check className="h-4 w-4" />
              Outcome attached to {recipient?.label}
            </div>
            <p className="mt-2 text-xs leading-5 text-[#405418]">
        ...
        <button onClick={onClose} className="mt-5 w-full rounded-2xl bg-[#20211F] py-3 text-sm font-semibold text-white">
          Close
        </button>
      </div>
    </div>
...
function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-b border-black/[0.05] p-4">
      <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-black/30">{title}</div>
...
function SpecLine({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <span className="font-medium text-black/65">{value}</span>
    </div>
  );
}
...
function MemoryPrinciple({ title, text }: { title: string; text: string }) {
  return (
    <div className="rounded-xl bg-[#F3F3EF] p-4">
      ...
      <p className="mt-2 text-[11px] leading-5 text-black/45">{text}</p>
    </div>
  );
}
